...
async function main() {
  ...
  try {
    ...
    const playerProgress = await progressCol.findOne({ discordId: PLAYER_ID });
    ...
    console.log("\n📢 公告內容已準備：");
    console.log("═══════════════════════════════════════");
    console.log("⚠️ **重大錯誤修正公告**");
    console.log("");
    console.log("親愛的玩家：");
    console.log("我們發現中級區及高級區經濟數值存在重大錯誤。");
    console.log("已完成修正，受影響玩家已發送補償道具。");
    console.log("");
    console.log("感謝耐心與配合！");
    console.log("═══════════════════════════════════════");
  } finally {
