...
async function simulate() {
  ...
  const monster = monsters[0]; // pick first
  const mCalc = calcStats(monster);

  // sample players (三組屬性)
  const players = [
    { name: '坦克', attrs: { str:4, agi:2, vit:8, int:1, dex:2, luk:1 } },
    { name: '輸出', attrs: { str:8, agi:6, vit:4, int:1, dex:4, luk:2 } },
    { name: '敏捷', attrs: { str:5, agi:10, vit:3, int:1, dex:6, luk:3 } }
  ];

  const statePlayers = players.map(p => {
    const stats = calcPlayerStats(p.attrs);
  ...
  for (let round=1; round<=MAX_ROUNDS; round++){
    ...
    // monster retaliation: attack random alive player
    ...
    for (let ma=0; ma<monsterAttackCount; ma++){
      ...
      if (Math.random()*100 < mhc){
        ...
        target.hp -= dmg;
        logs.push(`Monster hits ${target.name} for ${dmg} (hp ${Math.max(0,target.hp)})`);
