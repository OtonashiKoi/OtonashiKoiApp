...
async function findMonster(name){
  if (MONGO_URI){
    try{ const client = new MongoClient(MONGO_URI, { serverSelectionTimeoutMS: 5000 }); await client.connect(); const m = await client.db(DB_NAME).collection('monsters').findOne({ name }); await client.close(); if (m) return m; }catch(e){}
  }
...
async function findWoodenSword(){
  if (MONGO_URI){
    try{ const client = new MongoClient(MONGO_URI, { serverSelectionTimeoutMS: 5000 }); await client.connect(); const it = await client.db(DB_NAME).collection('items').findOne({ name: '木劍' }) || await client.db(DB_NAME).collection('items').findOne({ name: /木劍/ }); await client.close(); if (it) return it; }catch(e){}
  }
...
async function simulate(monsterName, playerCount = 4, equip = true){
  ...
  const sword = equip ? await findWoodenSword() : null;
  const equippedWeapon = sword ? Object.assign({}, sword) : null;

  const baseAttrs = { str:1, agi:1, vit:1, int:1, dex:1, luk:1 };
  const players = [];
  for (let i=0;i<playerCount;i++) players.push({ name: `P${i+1}`, attrs: baseAttrs });

  const statePlayers = players.map(p => {
    const stats = calcPlayerStats(p.attrs, equippedWeapon ? { weapon: equippedWeapon } : {});
  ...
  console.log(`Simulating vs monster: ${monster.name} (players=${playerCount}, equip=${equip})`);
...
const equip = process.argv[4] !== 'false';
simulate(monsterName, playerCount, equip).catch(e=>{ console.error(e); process.exit(1); });
