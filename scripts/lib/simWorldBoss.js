 ...
 */
async function createWorldBossSim(sc, db, zoneKey, part = null) {
  ...
  if (wbSvc) {
    ...
    const totalNow = Object.values(partsHp).reduce((s, v) => s + (Number(v) || 0), 0) || totalMax;
  ...
  return {
    ...
    info: `${boss.name}・${targetPart}｜部位HP ${partHp.toLocaleString()}｜atk${monsterStats.atk} def${monsterStats.def} flatDef${monsterStats.flatDef}${phase ? `｜階段 atk×${phase.atkMultiplier} def×${phase.defMultiplier}` : ""}`,
     ...
     */
    run(progress, { runs = 200, equipment = null, extraOptions = {} } = {}) {
      ...
      const acc = { dmg: 0, poison: 0, dotTotal: 0, deaths: 0, rounds: 0, hits: 0, taken: 0, wins: 0 };
