...
async function main() {
  ...
  const run = (eq, mc, mName, mEq, opts = {}) => {
    ...
    let maxStreak = 0, stunned = 0, mturns = 0, taken = 0;
    for (let i = 0; i < N; i++) {
    ...
    return { stunPct: stunned / mturns, taken: taken / N, maxStreak };
  };
  ...
  ck("巨神震擊場：怪物全程不出手", c.stunPct > 0.97, (c.stunPct * 100).toFixed(0) + "%");
