 ...
 *     本季 S 階只做武器（全遊戲 45 件 S 階裝備都是武器，不開 S 階防具先例）。
 *
 * 用法：
 *   node scripts/upsert-event-beach-gear.js              # 試跑，只印不寫
 *   node scripts/upsert-event-beach-gear.js --apply      # 寫入道具
 *   node scripts/upsert-event-beach-gear.js --apply --wire-drops   # 順便掛進怪物掉落表
 */
...
const GEAR = [
  ...
  { id: "beach-dice", name: "珊瑚骰", slot: "weapon", wType: "dice", atk: "luk", twoH: false, base: 23, stats: S(0, 4, 0, 0, 4, 15), desc: "浪打了三十年才磨成六面。點數據說偏大。" },
...
function buildItem(g, tier, elementDrop) {
  ...
  return {
    ...
    imageUrl: `${IMAGE_BASE}/${g.id}.png`,
    imageThumbnailUrl: `${IMAGE_BASE}/${g.id}.png`,
    enhanceLevel: 0,
    useEffects: [],
    passiveEffects: [],
    procEffects: [],
...
const BOSS_DROP_RATE = 100 / 3; // 龜王掉落表＝寶箱獎池，三件等機率
...
(async () => {
  ...
  if (WIRE_DROPS) {
    ...
    // 龜王：掉落表＝世界王寶箱獎池
    const boss = await monsters.findOne({ id: "event-island-turtle" });
    if (boss) {
      const bossDrops = BOSS_GEAR.map((g) => ({ itemId: g.id, chance: BOSS_DROP_RATE }));
      console.log(`  島島龜王（寶箱獎池）: ${(boss.drops || []).length} 項 → ${bossDrops.length} 項（各 ${BOSS_DROP_RATE.toFixed(1)}%）`);
      if (APPLY) await monsters.updateOne({ _id: boss._id }, { $set: { drops: bossDrops, updatedAt: new Date().toISOString() } });
    }
  }
