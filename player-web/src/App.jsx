  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '40px', textAlign: 'center' }}>
      ...
      <p style={{ marginTop: '24px', fontSize: '12px', color: 'var(--muted)' }}>
        ?ëÂÄëÂ?‰ΩøÁî® Discord OAuth2 ÂÆâÂÖ®?∞Â?Ê≠•ÊÇ®?ÑÈ??≤ÈÄ≤Â∫¶?áË??Ö„Ä?      </p>
    </div>
  );
}
  ...
  return (
    <div className="app-screen">
      <header style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <p style={{ margin: 0, fontSize: '13px', color: 'var(--muted)', fontWeight: 600 }}>?íÈö™?ÖÊ?Ê°?Profile</p>
          <h2 style={{ fontSize: '1.8rem' }}>{player.displayName}</h2>
        </div>
        <div style={{ textAlign: 'right' }}>
          <span style={{ display: 'inline-block', background: 'var(--accent-light)', color: 'var(--accent-strong)', padding: '4px 12px', borderRadius: '12px', fontSize: '14px', fontWeight: 'bold' }}>{progress.playerTier} Á¥öÁé©ÂÆ?/span>
        </div>
      </header>

      <div className="stat-grid" style={{ marginBottom: '20px' }}>
        <div className="stat-box">
          <small>?ëÂπ£ Gold</small>
          <strong>?í∞ {wallet.gold || 0}</strong>
        </div>
        <div className="stat-box">
          <small>?ΩÁü≥ Diamond</small>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', paddingBottom: '12px', borderBottom: '1px solid var(--glass-border)' }}>
          <span style={{ color: 'var(--muted)' }}>?∑Ê•≠ (Class)</span>
        ...
        <div style={{ background: 'var(--surface-hover)', padding: '16px', borderRadius: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <h4 style={{ margin: 0, fontSize: '14px' }}>Â±¨ÊÄßÈ?Èª?(Attributes)</h4>
            <span style={{ fontSize: '12px', background: 'var(--accent-strong)', color: '#fff', padding: '2px 8px', borderRadius: '10px' }}>?©È?Â±¨ÊÄßÈ?: {progress.statusPoints}</span>
          </div>
...
function InventoryTab() {
  ...
  if (!inventory || inventory.length === 0) {
    return (
      <div className="app-screen">
        <h2 style={{ marginBottom: '24px' }}>?åÂ? Inventory</h2>
        <div style={{ textAlign: 'center', color: 'var(--muted)', marginTop: '40px' }}>?åÂ?Á©∫Á©∫Â¶Ç‰?...</div>
      </div>
    );
  }
    ...
    <div className="app-screen">
      ...
      {inventory.map((item, idx) => (
        <div key={idx} className="list-item">
          ...
          <button className="btn" style={{ width: 'auto', padding: '4px 12px', fontSize: '12px', marginLeft: '12px' }}>?ç‰?</button>
        </div>
      ))}
    </div>
  );
...
function CombatTab() {
  ...
  const startBattle = async (zone) => {
    try {
      setIsBattling(true);
      setBattleState({ logs: ["?îÔ? Ê≠?ú®?çÂ??∞Â?..."], visibleLogs: ["?îÔ? Ê≠?ú®?çÂ??∞Â?..."] });
      ...
      setBattleState(prev => ({
        ...
        showResults: true,
        outcome: 'error',
        rewardLines: ["?†Ô? ?∫Êà∞?ºÁ??ØË™§Ôº?, String(err.message || "?™Áü•?üÂ?")]
      }));
      setIsBattling(false);
    }
  };
  ...
  return (
    <div className="app-screen" style={{ position: 'relative', height: '100%' }}>
      <h2 style={{ marginBottom: '16px' }}>?∞È¨•?Ä Combat</h2>
      <p style={{ color: 'var(--muted)', fontSize: '13px', marginBottom: '20px', lineHeight: 1.5 }}>
        ?∏Ê?‰Ω†ÊÉ≥?ëÊà∞?ÑÂú∞?ñÂ??ü„ÄÇÈö®?óÁ?Á¥öÊ??áÔ??Ø‰ª•Ëß???¥Âº∑Â§ßÁ??Ä?ü„Ä?      </p>

      {[ 
        { key: 'normal', name: '?∞Ê??Ä', lv: 'Lv.1 ~ 10', desc: '?©Â??ùÂ≠∏?ÖÁ??≤Ë?ÂßÜË?Â∞èÂ??™Áâ©?∫Ê??Ä?ü„Ä?, color: 'var(--success)' },
        { key: 'mid',    name: 'Ê¨°Á??Ä', lv: 'Lv.11 ~ 30', desc: '?¥Âç±?™Á??™Áâ©Â∑¢Á©¥ÔºåÂª∫Ë≠∞Ê??ôÂ•ΩË£ùÂ??çÂ?ÂæÄ??, color: 'var(--warn)' }
      ].map(z => {
            ...
            {data.monsterName && (
              <div style={{ background: 'rgba(0,0,0,0.2)', padding: '12px', borderRadius: '8px', border: '1px solid var(--glass-border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginBottom: '6px' }}>
                  <span style={{ fontWeight: 'bold', color: '#fff' }}>?ëæ {data.monsterName}</span>
        ...
        <div className="modal-overlay" onClick={(e) => {
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h3 style={{ fontSize: '1.1rem' }}>?îÔ? ?∞È¨•Á¥Ä??/h3>
              <button 
                ...
                {battleState.showResults && (
                  <div style={{ 
                    ...
                    <h4 style={{ color: battleState.outcome === 'win' ? '#f1c40f' : '#e74c3c', margin: '0 0 12px 0', fontSize: '1.2rem' }}>
                      {battleState.outcome === 'win' ? '?? ?∞È¨•?ùÂà©Ôº? : 
                       battleState.outcome === 'error' ? '??Á≥ªÁµ±?∞Â∏∏' :
                       battleState.outcome === 'lose' ? '?? ?∞È¨•Â§±Ê?' : '?∏Ô? ?∞Áï•?§ÈÄÄ'}
                    </h4>
...
function ShopTab() {
  ...
  const handleBuy = async (item) => {
    ...
    try {
      if (confirm(`Á¢∫Â?Ë¶ÅËä±Ë≤?${item.price} ?ëÂπ£Ë≥ºË≤∑ ${item.name} ?éÔ?`)) {
        await api.buyShopItem(item.id);
        alert(`?êÂ?Ë≥ºË≤∑ ${item.name}!`);
        loadData(); // Refresh list and wallet
      }
    } catch (err) {
      alert("Ë≥ºË≤∑Â§±Ê?: " + err.message);
    }
  };
  ...
  return (
    <div className="app-screen" style={{ paddingBottom: 'calc(var(--nav-height) + 16px)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h2 style={{ margin: 0 }}>?ÜÂ? Market</h2>
        {wallet && <div style={{ background: 'rgba(241, 196, 15, 0.2)', color: '#f1c40f', padding: '4px 12px', borderRadius: '12px', fontSize: '14px', fontWeight: 'bold' }}>?í∞ {wallet.gold}</div>}
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {items.length === 0 ? <p style={{ color: 'var(--muted)', textAlign: 'center', padding: '40px 0' }}>?ÆÂ?Ê≤íÊ?Ë≤©ÂîÆ‰ªª‰??ÜÂ?ÔºåË?Á®çÂ??ç‰???/p> : null}
          ...
          <div key={item.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              <div style={{ width: '40px', height: '40px', background: 'var(--surface-hover)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', overflow: 'hidden' }}>
                {item.imageUrl ? <img src={getAssetUrl(item.imageUrl)} alt="" style={{width: '100%', height: '100%', objectFit: 'cover'}} /> : '?ì¶'}
              </div>
              <div>
                ...
                <div style={{ fontSize: '11px', color: 'var(--success)', marginTop: '4px' }}>
                  {item.effect?.type === 'heal' ? `?¢Âæ© ${item.effect.value} HP` : 
                   item.equipStats ? `?ê‰?È°çÂ?Ë£ùÂ?Â±¨ÊÄßÂ??ê` : '?äÊà≤Ê∂àËÄóÂ?'}
                </div>
              </div>
            </div>
...
function ChatTab() {
  ...
  useEffect(() => {
    // ÂæûÁ¨¨‰∏Ä?ÅË??•Á??ÇÂÄôÂéª??ProfileÔºåÂ??∫ÈÄôË£°Ê≤íÊ??®Â??Ä?ãÔ??ëÂÄëÂÅ∑?∂Â??ø‰?Ê¨?    api.getProfile().then(data => {
      setMyDisplayName(data.player.displayName);
    }).catch(() => {});

    // 1. ?ñÂ?Ê≠∑Âè≤Á¥Ä??    api.getChatHistory().then(history => {
      const formatted = history.map(m => {
        // Â¶ÇÊ??ØÊ??®‰∫∫ËΩâÁôº Web ?ÑË??ØÔ?Ê™¢Êü•?ØÂê¶?ØÊ??ºÁ?
        let isMe = false;
        let text = m.content;
        let author = m.author;
        ...
        if (m.isBot && text.startsWith("[Web] **")) {
          ...
          if (match) {
            author = match[1];
            text = match[2];
            // ?ôË£°?Ñ‰??•È? myDisplayNameÔºåÊ?‰ª•Ê?ÈªûÊ∏≤?ìÊ??§Êñ∑
          }
        }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Optimistic upate (Ê≤?id ?ÉË¢´‰∏ã‰?Ê¨?SSE ?ãÊ??ñËøΩ?†Ô??ôË£°?à‰??®ÂÖ•?ªÈ??†Ê??ôÁ´ØÔºå‰??∫‰?ÊµÅÊö¢?ëÂÄëÂ???
  ...
  return (
    <div className="app-screen" style={{ height: '100%', paddingBottom: '16px' }}>
      <header style={{ flexShrink: 0, marginBottom: '8px' }}>
        <h2 style={{ margin: 0, fontSize: '1.4rem' }}>?éÈéÆÂ§ßÂª≥ Chat Lobby</h2>
        <p style={{ color: 'var(--muted)', fontSize: '12px', margin: '4px 0' }}>?ºÈÄÅÂ?Ë©±Ëá≥ Discord ?éÈéÆ?ªÈ?</p>
      </header>
      ...
      <div style={{ flex: 1, background: 'var(--bg)', border: '1px solid var(--line)', borderRadius: '16px', padding: '16px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px', boxShadow: 'inset 0 0 20px rgba(0,0,0,0.2)' }}>
        <div style={{ textAlign: 'center', color: 'var(--muted)', fontSize: '11px', marginBottom: '8px' }}>--- Ê≠∑Âè≤Á¥Ä??---</div>
        
        {messages.map((m, idx) => {
          const isMe = m.author === myDisplayName || m.author === "Me";
          
          // Ëß?? Discord Ë°®Ê?Á¨¶Ë??áÊ???          let parsedText = m.text.replace(/<a?:(\w+):(\d+)>/g, (match, name, id) => {
      ...
      <div style={{ marginTop: '12px', display: 'flex', gap: '8px', flexShrink: 0 }}>
        <input 
          ...
          placeholder="?ºÈÄÅË??ØËá≥ Discord..." 
          style={{ flex: 1, padding: '12px', borderRadius: '12px', background: 'var(--bg)', border: '1px solid var(--line)', color: '#fff' }} 
        />
        <button className="btn btn-primary" onClick={handleSend} style={{ width: 'auto', padding: '0 20px' }}>?ÅÂá∫</button>
      </div>
    </div>
  );
}
  ...
  useEffect(() => {
    ...
    if (authCode) {
      api.loginWithDiscord(authCode).then(data => {
        ...
        alert("?ªÂÖ•Â§±Ê?ÔºÅË?Á¢∫Ë??âÂ??ïÂ?Á´?API ‰º∫Ê???(npm start)Ôºö\n" + err.message);
        window.history.replaceState({}, document.title, "/");
        setIsInitializing(false);
      });
    } else {
