    ...
    const nodeRows = (editing.nodes || []).map((n, i) => {
      ...
      const mainArea = isChoice ? choiceArea : isBattle
        ? `<div style="${ROW}">
           ...
           <div style="${ROW}margin:6px 0 0;">
             <label style="font-size:12px;flex:1;min-width:220px;" title="橘色那行標題，留空＝自動用「戰鬥 怪物名」">⚔️ 戰鬥標題(橘字) <input type="text" data-node="${i}" data-field="battleTitle" value="${esc(n.battleTitle || "")}" placeholder="留空＝戰鬥 ${esc((monsters.find((m) => m.id === n.monsterId) || {}).name || "怪物")}" style="width:60%;min-width:140px;"></label>
           </div>
           <textarea data-node="${i}" data-field="text" rows="2" style="width:100%;box-sizing:border-box;margin-top:6px;" placeholder="戰鬥前的旁白/描述（選填，顯示在戰鬥框上方、點『出戰』前）">${esc(n.text || "")}</textarea>`
        : isCG
          ? `<div style="${ROW}margin-bottom:4px;">
    ...
    return `
      <div style="position:absolute;inset:0;${shakeAnim}">
        ...
        <div style="position:absolute;left:8px;right:8px;bottom:8px;z-index:5;padding:${ls ? "10px 12px" : "12px"};min-height:${P.boxMinH};background:linear-gradient(180deg,${isBattle ? "rgba(58,24,34,.96),rgba(24,12,20,.98)" : "rgba(30,24,58,.96),rgba(16,12,32,.98)"});border:1.5px solid ${isBattle ? "#ff5577" : n.type === "choice" ? "#ffd166" : "#c4a7f5"};border-radius:10px;">
          ${isBattle ? `${String(n.text || "").trim() ? `<div style="color:#f3ecff;font-size:13px;margin-bottom:6px;white-space:pre-wrap;">${esc(n.text)}</div>` : ""}<div style="text-align:center;color:#ff8a4a;font-weight:900;">⚔️ ${esc(n.battleTitle || ("戰鬥 " + ((monsters.find((m) => m.id === n.monsterId) || {}).name || "（未選怪）")))}</div><div style="text-align:center;color:#9b8cc0;font-size:10px;margin-top:2px;">🎯 ${esc((monsters.find((m) => m.id === n.monsterId) || {}).name || "未選怪")}${n.forcedOutcome === "win" ? "・劇情殺必勝" : n.forcedOutcome === "lose" ? "・劇情殺必敗" : ""}${n.maxRounds ? "・⏱" + esc(String(n.maxRounds)) + "回合" : ""}（後台提示，玩家看不到）</div>`
