...
(function () {
  ...
  async function loadItems(){const res=await fetch('/admin/items',{headers:authHeader()});const json=await res.json();if(json.status==='ok'){items=json.data||[];renderAll();}else{window.logActivity&&window.logActivity('???��?載入?�具庫�?'+(json.message||'')); }}
  ...
  function renderBody(){const tbody=document.getElementById('items-tbody');if(!tbody)return;const q=searchQuery.toLowerCase();const f=items.filter(i=>matchesTab(i,activeTab)&&(!q||(i.name||'').toLowerCase().includes(q)||(i.description||'').toLowerCase().includes(q)));if(!f.length){const c=TAB_COLS[activeTab];tbody.innerHTML=`<tr><td colspan="${c.length}" style="text-align:center;color:var(--muted);padding:2rem;">此�?類目?�為空�??�「�? ?��?一行」�?始建立�?/td></tr>`;return;}tbody.innerHTML=f.map((item,i)=>buildRow(item,false,i+1)).join('');bindRowEvents(tbody);}
  function esc(s){return String(s??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}
  function slotOptsHtml(sel){const mk=v=>`<option value="${v}"${v===sel?' selected':''}>${SLOT_LABELS[v]}</option>`;const std=['head_top','head_mid','head_low','armor','weapon','shield','garment','shoes','accessory_l','accessory_r'].map(mk).join('');const sp=['title_eq','job_eq','special_1','special_2','special_3'].map(mk).join('');return`<optgroup label="一?��???>${std}</optgroup><optgroup label="?��?槽�?">${sp}</optgroup>`;}
