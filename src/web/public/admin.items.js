/* admin.items.js */
(function () {
  let items=[],activeTab='consumable',pendingImgRowId=null;
  ...
  const TAB_COLS={consumable:['seq','img','name','desc','effect','effectValue','actions'],collectible:['seq','img','name','desc','actions'],equipment:['seq','img','name','desc','slot','str','agi','vit','int','dex','luk','actions'],special:['seq','img','name','desc','slot','str','agi','vit','int','dex','luk','actions']};
  const COL_HEADERS={
    seq:"#",
    img:"圖片",
    name:"名稱",
  ...
  function matchesTab(item,tab){if(tab==='consumable')return item.itemType==='consumable';if(tab==='collectible')return item.itemType==='collectible';if(tab==='equipment')return item.itemType==='equipment'&&STANDARD_SLOTS.has(item.equipSlot);if(tab==='special')return item.itemType==='equipment'&&SPECIAL_SLOTS.has(item.equipSlot);return true;}
  ...
  function getRowPayload(tr){const get=f=>tr.querySelector(`[data-field="${f}"]`)?.value??'';const isEq=activeTab==='equipment'||activeTab==='special';const p={name:get('name'),description:get('desc'),itemType:isEq?'equipment':defaultItemType(activeTab)};if(activeTab==='consumable')p.effect={type:get('effect')||'none',value:Number(get('effectValue'))||0};if(isEq){p.equipSlot=get('slot')||defaultSlot(activeTab);p.equipStats={str:Number(get('str'))||0,agi:Number(get('agi'))||0,vit:Number(get('vit'))||0,int:Number(get('int'))||0,dex:Number(get('dex'))||0,luk:Number(get('luk'))||0};}return p;}
  ...
  document.querySelectorAll('.items-tab-btn').forEach(btn=>{btn.addEventListener('click',()=>{activeTab=btn.dataset.itemsTab;document.querySelectorAll('.items-tab-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');renderAll();});});
  ...
  const CSV_COLS = {
    ...
    equipment:   ['name','description','equipSlot','str','agi','vit','int','dex','luk'],
  ...
  async function importCsv(file) {
    ...
    window.logActivity && window.logActivity(`✅ CSV 匹入完成：成功 ${ok} 筆，失敗 ${fail} 筆`);
    await loadItems();
  }
