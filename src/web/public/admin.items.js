...
(function () {
  ...
  const TAB_COLS={consumable:['seq','img','name','desc','effect','effectValue','actions'],collectible:['seq','img','name','desc','actions'],equipment:['seq','img','name','desc','slot','weaponType','tier','str','agi','vit','int','dex','luk','actions'],special:['seq','img','name','desc','slot','str','agi','vit','int','dex','luk','actions']};
  const COL_HEADERS={
    seq:"#",
    img:"圖片",
    name:"名稱",
  ...
  function matchesTab(item,tab){if(tab==='consumable')return item.itemType==='consumable';if(tab==='collectible')return item.itemType==='collectible';if(tab==='equipment'){if(!(item.itemType==='equipment'&&STANDARD_SLOTS.has(item.equipSlot)))return false;if(equipSubFilter!=='all'){const sg={weapon:['weapon','shield'],head:['head_top','head_mid','head_low'],defense:['armor','garment','shoes'],accessory:['accessory_l','accessory_r']};if(!(sg[equipSubFilter]||[]).includes(item.equipSlot))return false;}if(tierFilter!=='all'&&item.tier!==tierFilter)return false;return true;}if(tab==='special')return item.itemType==='equipment'&&SPECIAL_SLOTS.has(item.equipSlot);return true;}
  ...
  function getRowPayload(tr){const get=f=>tr.querySelector(`[data-field="${f}"]`)?.value??'';const isEq=activeTab==='equipment'||activeTab==='special';const draft=window.adminEffects?window.adminEffects.decodeState(get('effectDraft'),{useEffects:[],passiveEffects:[],procEffects:[],combatEffects:[]}):{useEffects:[],passiveEffects:[],procEffects:[],combatEffects:[]};const p={name:get('name'),description:get('desc'),itemType:isEq?'equipment':defaultItemType(activeTab),useEffects:draft.useEffects||[],passiveEffects:draft.passiveEffects||[],procEffects:draft.procEffects||[],combatEffects:draft.combatEffects||[]};if(activeTab==='consumable')p.effect={type:get('effect')||'none',value:Number(get('effectValue'))||0};if(isEq){p.effect={type:get('effect')||'none',value:Number(get('effectValue'))||0};p.equipSlot=get('slot')||defaultSlot(activeTab);p.equipStats={str:Number(get('str'))||0,agi:Number(get('agi'))||0,vit:Number(get('vit'))||0,int:Number(get('int'))||0,dex:Number(get('dex'))||0,luk:Number(get('luk'))||0};p.weaponType=(p.equipSlot==='weapon'||p.equipSlot==='shield')?(get('weaponType')||null):null;p.tier=get('tier')||null;}return p;}
