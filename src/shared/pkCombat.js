...
function attackerTurn({
  ...
  if (stunRoundsLeft <= 0 && Math.random() * 100 >= hitChance) {
    ...
    if (isCrit) {
      ...
      dmg = Math.round(rollDmg(Math.max(1, attackBase)) * critMult);
      if (atkStats.hasArcherBadge && atkStats.weaponType === "bow") {
        dmg = Math.round(dmg * (atkStats.archerBowDamageBoost || 1.2));
      }
    }
    if (isArcherCrit) dmg = Math.round(dmg * (atkStats.archerCritMultiplier || 1.5));
    ...
    log.push(`⚔️ ${critNote}${breakNote}**${atkName}** ${rand(atkVerbs)}，對 **${defName}** 造成 **${dmg}** 點傷害！（${defName} 剩 ${Math.max(0, defHpRef.value)} HP）`);

    // 吸血
    if (lifestealPct > 0) {
      const heal = Math.max(1, Math.round(dmg * lifestealPct / 100));
    ...
    if (!killed) {
      ...
      for (const pe of allProcs) {
        ...
        switch (pe.key) {
          case 'burn':
            defActive = addOrStackEffect(defActive, {
              key: 'burn', params: { value: pp.value ?? 0.5, mode: pp.mode ?? 'pct', duration: dur },
          ...
          case 'hit_down':
            defActive = addOrStackEffect(defActive, {
              key: 'hit_rate_down', params: { value: pp.value ?? 15, duration: dur },
          ...
          case 'freeze':
            defActive = addOrStackEffect(defActive, {
              key: 'freeze', params: { duration: dur },
          ...
          case 'proc_poison': {
            ...
            defActive = addOrStackEffect(defActive, {
              key: 'poison', params: { value: nextPct, mode: 'pct', duration: dur },
    ...
    if (!killed) {
      ...
      for (const slot of specialSlots) {
        const slotItem = atkOpts.equipped?.[slot];
        if (!slotItem?.monsterCardSkill?.key) continue;
        ...
        log.push(`🎴 【${skill.name || cardName}】發動！${skill.description || ""}`);
...
function applyDotEffects({ name, hpRef, maxHp, activeEffects, round, log }) {
  let dead = false;
  let frozen = false;
  let silenced = false;
  ...
  for (const eff of activeEffects) {
    ...
    if (eff.key === 'burn') {
      ...
      hpRef.value -= dmg;
      log.push(`🔥 燒傷持續！**${name}** 受到 **${dmg}** 點灼燒傷害！（${name} 剩 ${Math.max(0, hpRef.value)} HP）`);
      if (hpRef.value <= 0) dead = true;
    }
    if (eff.key === 'poison') {
      const dmg = Math.max(1, Math.round(base * (Number(p.value ?? 0.5) / 100)));
      hpRef.value -= dmg;
      log.push(`☠️ 中毒持續！**${name}** 受到 **${dmg}** 點毒素傷害！（${name} 剩 ${Math.max(0, hpRef.value)} HP）`);
      if (hpRef.value <= 0) dead = true;
    }
    if (eff.key === 'bleed') {
      const dmg = Math.max(1, Math.round(base * (Number(p.value ?? 0.1) / 100)));
      hpRef.value -= dmg;
      log.push(`🩸 流血持續！**${name}** 受到 **${dmg}** 點流血傷害！（${name} 剩 ${Math.max(0, hpRef.value)} HP）`);
      if (hpRef.value <= 0) dead = true;
    }
    if (eff.key === 'lightning') {
      const dmg = Math.max(1, Math.round(base * (Number(p.value ?? 0.2) / 100)));
      hpRef.value -= dmg;
      log.push(`⚡ 閃電持續！**${name}** 受到 **${dmg}** 點雷電傷害！（${name} 剩 ${Math.max(0, hpRef.value)} HP）`);
      if (hpRef.value <= 0) dead = true;
    }
...
function runPkCombat(aStats, aOpts, aName, bStats, bOpts, bName, MAX_ROUNDS = 15) {
  ...
  for (let round = 1; round <= MAX_ROUNDS; round++) {
    ...
    // ── A 的 DOT / Buff / 眩暈 ─────────────────────────────
    const aDot = applyDotEffects({ name: aName, hpRef: aHpRef, maxHp: aStats.maxHp, activeEffects: aActive, round, log });
    if (aDot.dead) { winner = "B"; roundLogs.push(log.join("\n")); break; }

    // ── B 的 DOT / Buff / 眩暈 ─────────────────────────────
    const bDot = applyDotEffects({ name: bName, hpRef: bHpRef, maxHp: bStats.maxHp, activeEffects: bActive, round, log });
    ...
    if (!aDot.frozen) {
      const r = attackerTurn({
        ...
        defStats: bStats, defName: bName, defHpRef: bHpRef, defActive: bActive,
    ...
    if (!bDot.frozen) {
      const r = attackerTurn({
        ...
        defStats: aStats, defName: aName, defHpRef: aHpRef, defActive: aActive,
