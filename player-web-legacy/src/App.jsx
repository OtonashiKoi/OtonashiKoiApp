...
function GameHomeTab({ onNavigate, unreadCount = 0, onOpenNotif, onOpenWeekly }) {
  const [profile, setProfile] = useState(null);
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      api.getCombatZones(),
      api.getWeeklyQuests(),
    ]).then(([profileResult, zonesResult, questsResult]) => {
  ...
  const gold = profile?.wallet?.gold || 0;
  const diamond = profile?.wallet?.diamond || 0;
  const tier = profile?.progress?.playerTier || 'E';
  const exp = profile?.progress?.exp || 0;
  const nextLevelExp = profile?.progress?.nextLevelExp || 0;
  const expPct = nextLevelExp > 0 ? Math.max(0, Math.min(100, Math.round((exp / nextLevelExp) * 100))) : 100;
  const combatStats = calcPlayerStats(profile?.progress?.attributes, profile?.progress?.equipment);
  const powerScore = Math.round((combatStats.atk || 0) + (combatStats.def || 0) * 3 + (combatStats.maxHp || 0) * 0.5);

  const normalZone = zones.find((zone) => zone.zone === 'normal');
  const midZone = zones.find((zone) => zone.zone === 'mid');
  const recommendedZone = level >= 10 && midZone ? midZone : normalZone || midZone || null;
  const zoneLabel = recommendedZone?.zone === 'mid' ? '中級獵區' : '初心獵區';
  const zoneFlavor = recommendedZone?.zone === 'mid'
    ? '怪物強度明顯上升，掉寶與經驗也更值得冒險。'
    : '最適合穩定刷裝、熟悉循環與衝等的戰區。';
  const monsterHpPct = recommendedZone?.maxHp > 0
    ? Math.max(0, Math.min(100, Math.round((recommendedZone.currentHp / recommendedZone.maxHp) * 100)))
    : 0;

  const activeQuest = quests.find((entry) => !entry.claimed && !entry.done)
    || quests.find((entry) => entry.done && !entry.claimed)
    || quests[0];
  const activeQuestPct = activeQuest
    ? Math.max(0, Math.min(100, Math.round((activeQuest.current / Math.max(activeQuest.quest.target, 1)) * 100)))
    : 0;
  const nextGoalText = level < 10
    ? `再升 ${10 - level} 級即可解鎖中級獵區。`
    : '你已解鎖中級獵區，下一步是把裝備與週任務效率一起拉高。';

  const quickActions = [
    { label: '開始狩獵', sublabel: zoneLabel, onClick: () => onNavigate('combat'), primary: true },
    { label: '整理裝備', sublabel: '背包與穿裝', onClick: () => onNavigate('inventory') },
    { label: '補給商店', sublabel: '購買與強化素材', onClick: () => onNavigate('shop') },
    { label: '任務簡報', sublabel: activeQuest ? `${activeQuest.current}/${activeQuest.quest.target}` : '查看進度', onClick: onOpenWeekly },
  ];

  const urgentObjectives = [
    {
      eyebrow: 'FIELD',
      title: recommendedZone?.monsterName ? `討伐 ${recommendedZone.monsterName}` : '選擇下一場戰鬥',
      body: recommendedZone?.monsterName
        ? `${zoneLabel} 的主目標仍在場上，現在進場最容易把這次狩獵推進成一次完整回報。`
        : '目前沒有偵測到主目標，先前往戰鬥頁刷新戰區狀態。',
      action: '前往戰鬥',
      onClick: () => onNavigate('combat'),
    },
    {
      eyebrow: 'QUEST',
      title: activeQuest?.quest?.title || '查看本週任務',
      body: activeQuest
        ? `進度 ${activeQuest.current}/${activeQuest.quest.target}，這是現在最接近轉成獎勵的一條成長線。`
        : '目前沒有讀到任務資料，打開週任務頁檢查可領獎勵與追蹤目標。',
      action: '打開週任務',
      onClick: onOpenWeekly,
    },
    {
      eyebrow: 'POWER',
      title: `戰力評估 ${powerScore.toLocaleString()}`,
      body: nextGoalText,
      action: '整理裝備',
  ...
  return (
    <div className="home-shell">
      ...
      <div className="home-topbar">
        <div>
          ...
          <div className="home-identity">
            <h2>{name}</h2>
            <span className="home-tier-badge">Tier {tier}</span>
          </div>
          <div className="home-level-row">
            <span className="home-level-chip">LV. {level}</span>
            <span className="home-level-chip home-level-chip-muted">{expPct}% to next level</span>
          </div>
        </div>

        <div className="home-resource-stack">
          <button className="home-notif-btn" onClick={onOpenNotif}>
            <span>通知</span>
            {unreadCount > 0 && (
              <span className="notif-badge">{unreadCount > 9 ? '9+' : unreadCount}</span>
            )}
          ...
          <div className="home-resource-pill home-resource-pill-blue">
            <span>Diamond</span>
            <strong>{diamond.toLocaleString()}</strong>
          </div>
        </div>
      </div>
      ...
      <div className="home-main-panel">
        <div className="home-directive-card">
          <div className="home-section-label">Field Directive</div>
          <div className="home-directive-header">
            <div>
              <h3>{recommendedZone?.monsterName || '準備下一場狩獵'}</h3>
              <p>{zoneLabel} · {zoneFlavor}</p>
            </div>
            <div className="home-threat-chip">
              <span>威脅目標</span>
              <strong>{recommendedZone?.monsterLevel ? `Lv.${recommendedZone.monsterLevel}` : '待命中'}</strong>
            </div>
          </div>

          <div className="home-directive-progress">
            <div className="home-progress-head">
              <span>{recommendedZone?.monsterName ? '目標生命' : '戰區狀態'}</span>
              <strong>
                {recommendedZone?.monsterName
                  ? `${recommendedZone.currentHp} / ${recommendedZone.maxHp}`
                  : '尚未取得資料'}
              </strong>
            </div>
            <div className="home-progress-bar">
              <div style={{ width: `${monsterHpPct}%` }} />
            </div>
          </div>
          ...
          <div className="home-action-grid">
            {quickActions.map((action) => (
              <button
                key={action.label}
                className={`home-action-btn ${action.primary ? 'primary' : ''}`}
        ...
        <div className="home-summary-grid">
          <div className="home-summary-card">
            <span>戰力評估</span>
            <strong>{powerScore.toLocaleString()}</strong>
            <small>ATK / DEF / HP 綜合換算</small>
          </div>
          <div className="home-summary-card">
            <span>升級進度</span>
            <strong>{nextLevelExp > 0 ? `${exp} / ${nextLevelExp}` : 'MAX'}</strong>
            <small>{nextGoalText}</small>
          </div>
          <div className="home-summary-card">
            <span>週任務焦點</span>
            <strong>{activeQuest ? `${activeQuest.current}/${activeQuest.quest.target}` : '待確認'}</strong>
            <small>{activeQuest?.quest?.title || '打開週任務檢查本週目標'}</small>
          </div>
        </div>

        <div className="home-objectives-card">
          <div className="home-section-label">First Minute Loop</div>
          <div className="home-objective-list">
            {urgentObjectives.map((objective) => (
              <button key={objective.title} className="home-objective-row" onClick={objective.onClick}>
                <div>
                  <span>{objective.eyebrow}</span>
                  <strong>{objective.title}</strong>
                  <p>{objective.body}</p>
                </div>
                <em>{objective.action}</em>
              </button>
            ))}
          </div>
        </div>

        <div className="home-dialogue-card" onClick={handleDialogueClick}>
          <div className="home-section-label">Camp Whisper</div>
          <p>{displayed}</p>
          {done && <span className="home-dialogue-tip">tap to continue</span>}
        </div>

        {activeQuest && (
          <div className="home-quest-card">
            <div className="home-quest-header">
              <div>
                <span>Weekly Objective</span>
                <strong>{activeQuest.quest.title}</strong>
              </div>
              <button className="btn" style={{ width: 'auto', padding: '9px 14px' }} onClick={onOpenWeekly}>
                查看任務
              </button>
            </div>
            {activeQuest.quest.description && <p>{activeQuest.quest.description}</p>}
            <div className="home-progress-head">
              <span>目前進度</span>
              <strong>{activeQuest.current} / {activeQuest.quest.target}</strong>
            </div>
            <div className="home-progress-bar home-progress-bar-quest">
              <div style={{ width: `${activeQuestPct}%` }} />
            </div>
          </div>
        )}
      </div>
...
function JRPGCombatTab({ onNavigate }) {
  const [profile, setProfile] = useState(null);
  const [zones, setZones] = useState([]);
  ...
  const fetchSceneData = () => {
    Promise.all([api.getProfile(), api.getCombatZones()])
      .then(([profileData, zoneData]) => {
        setProfile(profileData);
        setZones(zoneData);
        if (!zoneData.some(zone => zone.zone === selectedZone) && zoneData[0]?.zone) {
          setSelectedZone(zoneData[0].zone);
        }
        const serverCd = zoneData.find(z => z.nextBattleAt)?.nextBattleAt ?? null;
  ...
  useEffect(() => {
    fetchSceneData();
    const timer = setInterval(fetchSceneData, 7000);
    return () => clearInterval(timer);
  }, []);
  ...
  const playerStats = calcPlayerStats(progress.attributes, progress.equipment);
  const selectedZoneData = zones.find(zone => zone.zone === selectedZone) || zones[0] || null;
  const monsterMaxHp = selectedZoneData?.maxHp || 1;
  const currentMonsterHp = battleScene?.monsterHp ?? selectedZoneData?.currentHp ?? monsterMaxHp;
  const currentPlayerHp = battleScene?.playerHp ?? playerStats.maxHp;
  const monsterHpPct = Math.max(0, Math.min(100, Math.round((currentMonsterHp / Math.max(1, monsterMaxHp)) * 100)));
  const playerHpPct = Math.max(0, Math.min(100, Math.round((currentPlayerHp / Math.max(1, playerStats.maxHp || 1)) * 100)));
  const zoneThemes = {
    normal: {
      className: 'battle-scene-theme-frost',
      title: 'Frostpeak Crag',
      subtitle: 'Beginner Hunt Route',
    },
    mid: {
      className: 'battle-scene-theme-depths',
      title: 'Leviathan Depths',
      subtitle: 'Advanced Hunt Route',
    },
  };
  const zoneTheme = zoneThemes[selectedZoneData?.zone] || {
    className: 'battle-scene-theme-frost',
    title: selectedZoneData?.monsterName || 'Frontier Gate',
    subtitle: 'Combat Zone',
  };
  const isOnCooldown = nextBattleAt && nextBattleAt > Date.now();

  const startBattle = async () => {
    if (!selectedZoneData || isBattling || isOnCooldown) return;

    const initialMonsterHp = selectedZoneData.currentHp || selectedZoneData.maxHp || 1;
    const initialPlayerHp = playerStats.maxHp || 1;

    setIsBattling(true);
    setBattleFeed(['Battle started.']);
    setBattleScene({
      playerHp: initialPlayerHp,
      playerMaxHp: initialPlayerHp,
      monsterHp: initialMonsterHp,
      monsterMaxHp: selectedZoneData.maxHp || initialMonsterHp,
      outcome: null,
      totalDamage: 0,
      rewardSummary: null,
    });
    ...
    try {
      const result = await api.quickBattle(selectedZone);
      if (result.nextBattleAt) setNextBattleAt(result.nextBattleAt);

      const logs = Array.isArray(result.logs) ? result.logs : [];
      const monsterDamage = Math.max(0, initialMonsterHp - (result.finalMonsterHp ?? 0));
      const playerDamage = Math.max(0, initialPlayerHp - (result.finalPlayerHp ?? 0));
      const totalSteps = Math.max(1, logs.length);
      ...
      const interval = setInterval(() => {
        ...
        setBattleFeed(prev => {
          const next = [...prev];
          if (logs[step - 1]) next.push(logs[step - 1]);
          return next.slice(-5);
        });

        setBattleScene(prev => prev ? ({
          ...prev,
          playerHp: Math.max(result.finalPlayerHp ?? 0, Math.round(initialPlayerHp - playerDamage * ratio)),
          monsterHp: Math.max(result.finalMonsterHp ?? 0, Math.round(initialMonsterHp - monsterDamage * ratio)),
        }) : prev);
        ...
        if (step >= totalSteps) {
          ...
          setBattleScene(prev => prev ? ({
            ...prev,
            playerHp: Math.max(0, result.finalPlayerHp ?? 0),
            monsterHp: Math.max(0, result.finalMonsterHp ?? 0),
            outcome: result.outcome,
            totalDamage: result.totalDamage || 0,
            rewardSummary: result.rewardSummary || null,
          }) : prev);
          setIsBattling(false);
          fetchSceneData();
        }
      ...
      setBattleFeed([err.message || 'Battle failed.']);
      setBattleScene(prev => prev ? ({ ...prev, outcome: 'error' }) : prev);
      setIsBattling(false);
    }
  };

  const skillSlots = [
    { label: 'Slash', ready: true },
    { label: 'Burst', ready: true },
    { label: 'Skill 3', ready: false },
    { label: 'Skill 4', ready: false },
    { label: 'Skill 5', ready: false },
  ];

  return (
    <div className={`battle-scene-shell ${zoneTheme.className}`}>
      <div className="battle-scene-backdrop" />
      <div className="battle-scene-stars" />

      <header className="battle-topbar">
        <button className="battle-avatar-card" onClick={() => onNavigate('profile')}>
          <div className="battle-avatar-frame">
            <img src={`${import.meta.env.BASE_URL}chara.png`} alt="Hero" />
          </div>
          <div className="battle-avatar-meta">
            <strong>{player.displayName || 'Hunter'}</strong>
            <span>LV {progress.level || 1} · Tier {progress.playerTier || 'E'}</span>
          </div>
        </button>

        <div className="battle-currency-stack">
          <div className="battle-currency-pill">
            <span>Gold</span>
            <strong>{(wallet.gold || 0).toLocaleString()}</strong>
          </div>
          <div className="battle-currency-pill battle-currency-pill-alt">
            <span>Diamond</span>
            <strong>{(wallet.diamond || 0).toLocaleString()}</strong>
          </div>
        </div>
      </header>
      ...
      <div className="battle-map-row">
        {zones.map(zone => {
          const active = zone.zone === selectedZone;
          const theme = zoneThemes[zone.zone] || zoneTheme;
          return (
            <button
              key={zone.zone}
              className={`battle-map-chip ${active ? 'active' : ''}`}
      ...
      <div className="battle-stage-card">
        <div className="battle-stage-ring battle-stage-ring-outer" />
        <div className="battle-stage-ring battle-stage-ring-inner" />

        <div className="battle-stage-header">
          <div>
            <span className="battle-stage-label">Current Hunt</span>
            <h2>{selectedZoneData?.monsterName || 'Awaiting Monster'}</h2>
            <p>{zoneTheme.title} · {zoneTheme.subtitle}</p>
          </div>
          <div className="battle-stage-badges">
            <span>Lv {selectedZoneData?.monsterLevel || 1}</span>
            <span>{selectedZoneData?.participantCount || 0} hunters</span>
          </div>
        </div>

        <div className="battle-hp-panel enemy">
          <div className="battle-hp-head">
            <span>Enemy HP</span>
            <strong>{currentMonsterHp} / {monsterMaxHp}</strong>
          </div>
          <div className="battle-hp-bar">
            <div className="enemy-fill" style={{ width: `${monsterHpPct}%` }} />
          </div>
        </div>

        <div className="battle-monster-stage">
          <div className="battle-stage-platform" />
          {selectedZoneData?.monsterImageUrl ? (
            <img
              className={`battle-monster-art ${isBattling ? 'is-battling' : ''}`}
              src={getAssetUrl(selectedZoneData.monsterImageUrl)}
              alt={selectedZoneData.monsterName || 'Monster'}
            />
          ) : (
            <div className="battle-monster-fallback">?</div>
          )}
          <div className="battle-start-orb-wrap">
            <button
              className={`battle-start-orb ${isBattling ? 'is-battling' : ''}`}
              onClick={startBattle}
              disabled={!selectedZoneData || isBattling || isOnCooldown}
            >
              <span>{isBattling ? 'Clash' : 'Start'}</span>
              <strong>{isOnCooldown ? `${cdSecs}s` : 'Battle'}</strong>
            </button>
          </div>
        </div>

        <div className="battle-hp-panel player">
          <div className="battle-hp-head">
            <span>Your HP</span>
            <strong>{currentPlayerHp} / {playerStats.maxHp || 1}</strong>
          </div>
          <div className="battle-hp-bar">
            <div className="player-fill" style={{ width: `${playerHpPct}%` }} />
          </div>
        </div>

        <div className="battle-skill-row">
          {skillSlots.map((slot, index) => (
            <button
              key={slot.label}
              className={`battle-skill-slot ${slot.ready ? 'ready' : 'locked'}`}
              onClick={() => !slot.ready && onNavigate('inventory')}
            >
              <span>{slot.ready ? `S${index + 1}` : '+'}</span>
              <small>{slot.label}</small>
            </button>
          ))}
        </div>

        <div className="battle-feed-card">
          <div className="battle-feed-header">
            <span>Battle Feed</span>
            <strong>
              {battleScene?.outcome === 'win' ? 'Victory' :
                battleScene?.outcome === 'lose' ? 'Defeat' :
                battleScene?.outcome === 'error' ? 'Interrupted' :
                isBattling ? 'Engaged' : 'Ready'}
            </strong>
          </div>
          <div className="battle-feed-list">
            {(battleFeed.length ? battleFeed : [
              'Choose a map, lock onto the monster, and strike the first blow.',
              'Skill slots are reserved for future abilities and combo systems.',
            ]).map((line, idx) => (
              <p key={`${line}-${idx}`}>{line}</p>
            ))}
          </div>
          <div className="battle-reward-strip">
            <span>Damage {battleScene?.totalDamage || 0}</span>
            <span>Reward {(selectedZoneData?.goldReward || 0).toLocaleString()} G</span>
            <span>EXP {(selectedZoneData?.expReward || 0).toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column' }}>
        {activeTab === 'home' && <GameHomeTab onNavigate={setActiveTab} unreadCount={unreadCount} onOpenNotif={() => { setUnreadCount(0); setShowNotifModal(true); }} onOpenWeekly={() => setShowWeeklyTab(true)} />}
        {activeTab === 'profile' && <ProfileTab onOpenSettings={() => setShowSettingsModal(true)} />}
        {activeTab === 'inventory' && <InventoryTab />}
        {activeTab === 'combat' && <JRPGCombatTab onNavigate={setActiveTab} />}
      ...
      <nav className="bottom-nav">
        ...
        <div className={`nav-item nav-item-battle ${activeTab === 'combat' ? 'active' : ''}`} onClick={() => setActiveTab('combat')}>
          <Icons.Swords />
          <span>戰鬥</span>
        </div>
