...
### 🕳️ 踩過的坑：戰報顯示與實戰倍率脫鉤（2026-08-19 已修）

- **現象**：玩家水2武器打火2怪，攻擊姿態實戰確實吃到 ×1.4（+40%），
  但第一回合戰報印「水2 剋 火 → 對敵傷害 +20%」（玩家回報：討論串 1539644111429304400）。
- **根因**：實戰倍率走 `stanceElementMult`（`combatLoop.js:1392`），
  但屬性判定的戰報行是**另外**用「武器等級×10%」算的，沒看姿態升級後的等級。
  無屬性武器（實戰+20%卻顯示±0%）、被剋武器（實戰+40%卻顯示−N%）同樣中招。
- **修法**：戰報在 `stanceElementMult` 生效時改印姿態專屬行
  「攻擊（攻擊姿態）｜水2 升為 4 級剋 火 → 對敵傷害 +40%」，顯示與實戰用同一個 `stanceGuaranteedLevel`
  ✅（`src/shared/combatLoop.js:1795` 附近，屬性判定區塊）。
