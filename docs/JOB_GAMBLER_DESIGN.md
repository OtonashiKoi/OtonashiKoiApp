...
> 狀態：**骰子、賭徒與二轉賭神均已實作；DB 的賭徒／賭神任務目前啟用**（2026-08-10 核對）。本文的早期平衡表只作設計沿革。
> 範圍：第 11 個一轉職業「賭徒」、新武器類型 `dice`（骰子）
> 二轉現況：賭神 `job_dicegod_t2_v1` 已存在；二轉持有上限已取消，費用依持有數遞增。
>
> 版本沿革：
...
> 原先討論的 `casino_spend`（賭場累積下注）未採用；一轉與二轉目前都不使用此 metric。
...
| enabled | **`true`**（DB 與 default seed 已同步） |

### ⛔ 作廢：`casino_spend`（賭場累積下注 500 萬）方案

原提案「以賭場累積下注 500 萬解鎖」**未採用**；DB 的「賭神試煉」為 `type: t2_transfer`，`casino_spend` metric 沒有實作，也不是現行待辦。
...
2. Lv.10 且基礎 LUK + AGI > 10 —— 任務出現
3. 使用骰子出戰 10 次，進度即時累積
